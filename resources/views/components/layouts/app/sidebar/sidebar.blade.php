<aside class="flex-shrink-0 flex flex-col w-3/12 max-w-80 bg-secondary-100 border-r border-secondary-200 h-dvh overflow-y-auto py-4">
    <a href="{{ url('/') }}" class="flex gap-x-2 mb-10 mx-6">
        <div class="bg-primary flex justify-center items-center aspect-square size-10 overflow-hidden rounded-sm">
    ...
    <div class="flex flex-col mx-4 mb-6">
        <x-layouts.app.sidebar.sidebar-item class="!h-11" :active="request()->routeIs('dashboard')">
            <x-slot:icon class="flex items-center justify-center">
                <x-phosphor-browser-duotone class="size-5.5" />
            </x-slot:icon>

            <span class="grow">Dashboard</span>
        </x-layouts.app.sidebar.sidebar-item>

        <x-layouts.app.sidebar.sidebar-item class="!h-11">
            <x-slot:icon class="flex items-center justify-center">
